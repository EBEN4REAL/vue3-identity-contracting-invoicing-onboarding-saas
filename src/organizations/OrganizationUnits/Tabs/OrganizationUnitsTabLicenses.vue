...
const emptyState: EmptyStateType = {
  ...
  title: "Add license to unit",
  description:
    "All users in this organizational unit can then access and use the license.",
  learnMoreHref:
...
const licenses: Ref<TableResponse<SCAgreementRead> | null> = ref(null);
...
const description: string = "All licenses directly assigned to this unit.";
...
const isLicenseBtnDisabled: ComputedRef<boolean> = computed(
  () => !licenses.value?.results || props.isSCViewerOnly,
);
...
const submit = async (payload: SubmitType) => {
  try {
    ...
    alertText.value = "Failed to add license to unit";
  }
};
