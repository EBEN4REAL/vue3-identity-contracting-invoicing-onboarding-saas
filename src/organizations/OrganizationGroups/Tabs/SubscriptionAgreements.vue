...
const emptyState: EmptyStateType = {
  ...
  title: "Add subscription to group",
  description:
    "All users in this group can then access and use the subscription.",
  learnMoreHref:
...
const subscriptionToUnassign: Ref<TypeLicenseItemInRow | null> = ref(null);
const loadingUnassign: Ref<boolean> = ref(false);
const isLoading: Ref<boolean> = ref(true);
const agreements: Ref<ServiceConsumerAgreementRead[]> = ref([]);
const description: string = "All subscriptions assigned to this group.";

const assignedSubscriptions: ComputedRef<TypeFormattedLicenses[]> = computed(
  () =>
    subscriptions.value?.results.map((subscription) => ({
      id: subscription.id,
      name: subscription.agreement_type_name,
      description: subscription.description,
      has_optional_policies: subscription.has_optional_policies,
    })) || [],
);

const title: ComputedRef<string> = computed(
  () => `Remove ${subscriptionToUnassign.value?.name}`,
);

const subtitle: ComputedRef<string> = computed(
  () =>
    `Are you sure you want to remove the subscription from the ${props.group.name} group?`,
);
...
const handleUpdateParams = async (params?: TableRequestParams) => {
  ...
  await getSubscriptionAgreements(params);
};
...
const getSubscriptionAgreements = async (params?: TableRequestParams) => {
  ...
  subscriptions.value = await policiesService.getServiceConsumerGroupAgreements(
...
const onUnassignButtonClick = async () => {
  if (subscriptionToUnassign.value) {
...
<template>
  ...
  <assign-license-to-group
    ...
    :assigned-licenses="assignedSubscriptions"
    category="SUBSCRIPTION"
    @update-org-licenses="updateOrgSubscriptions"
    @close="toggleAssignDialog"
    @submit="getSubscriptionAgreements"
  />
