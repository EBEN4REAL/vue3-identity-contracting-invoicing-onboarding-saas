    ...
    if (props.isOpen) {
      if (props?.user?.id) {
        const policies: ServiceConsumerAgreementPoliciesRead =
          await policiesService.getServiceConsumerAgreementPoliciesAssignedToUser(
      ...
      if (!isPoliciesFetched.value) {
        try {
          ...
          eventBus.$emit("onShowToast", {
            text: `Failed to fetch optional license policies for ${props.user?.name}`,
            status: "error",
          });
        }
      }
...
const title: ComputedRef<string> = computed(
  () => `Manage optional policies for ${props.user?.name}`,
);
...
const onSubmit = async () => {
  try {
    ...
    if (props.user?.id) {
      await policiesService.updateServiceConsumerAgreementOptionalPoliciesAssignedToUser(
        props.serviceConsumerId,
        props.licenseId,
        props.user?.id,
...
<template>
  <m-m-dialog
    ...
    cy="dialog-manage-optional-licenses-for-user"
    @close="emit('close')"
  >
    <template #default>
    ...
    <template #footer>
      <div class="mm-flex mm-justify-flex-end mm-flex-gap-6">
        ...
        <m-m-button
          cy="button-submit-manage-optional-policies-for-user"
