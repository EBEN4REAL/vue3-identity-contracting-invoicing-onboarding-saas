...
const subtitle: ComputedRef<string> = computed(
  () =>
    `Are you sure you want to remove ${props.user?.name} from this license?`,
);
...
const onSubmit = async () => {
  try {
    ...
    eventBus.$emit("onShowToast", {
      text: "Failed to unassign license from user",
      status: "error",
    });
  } finally {
...
<template>
  <m-m-dialog
    ...
    icon="error"
    icon-color="error"
    cy="dialog-unassign-license-from-user"
    @close="emit('close')"
  >
    <template #footer>
      <div class="mm-flex mm-justify-flex-end mm-flex-gap-6">
        ...
        <m-m-button
          cy="button-unassign-license-from-user"
          variant="danger"
