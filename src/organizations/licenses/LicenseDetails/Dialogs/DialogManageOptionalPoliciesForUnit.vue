    ...
    if (props.isOpen) {
      if (props?.unit?.id) {
        const policies: ServiceConsumerAgreementPoliciesRead =
          await policiesService.getServiceConsumerAgreementPoliciesAssignedToUnit(
      ...
      if (!isPoliciesFetched.value) {
        try {
          ...
          eventBus.$emit("onShowToast", {
            text: `Failed to fetch optional policies for ${props.unit?.name}`,
            status: "error",
          });
        }
      }
...
const title: ComputedRef<string> = computed(
  () => `Manage Optional Policies for ${props.unit?.name}`,
);
...
const onSubmit = async () => {
  try {
    ...
    if (props.unit?.id) {
      await policiesService.updateServiceConsumerAgreementOptionalPoliciesAssignedToUnit(
        props.serviceConsumerId,
        props.licenseId,
        props.unit?.id,
...
<template>
  <m-m-dialog
    ...
    cy="dialog-manage-optional-licenses-for-unit"
    @close="emit('close')"
  >
    <template #default>
    ...
    <template #footer>
      <div class="mm-flex mm-justify-flex-end mm-flex-gap-6">
        ...
        <m-m-button
          cy="button-submit-manage-optional-policies-for-unit"
