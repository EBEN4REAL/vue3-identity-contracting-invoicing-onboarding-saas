...
const title = computed(() => `Remove ${fullName.value}`);

const subtitle = computed(
  () =>
    `Are you sure you want to remove ${fullName.value}? This will affect their access to this organization - ${props.data?.organization?.name}.`,
);
...
const submit = async () => {
  if (props.data?.organization?.id && props.data?.user_id) {
    try {
      ...
      await organizationsService.deleteOrganizationUser(
