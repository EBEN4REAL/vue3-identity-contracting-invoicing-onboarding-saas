...
const title = computed(() => `Disable ${fullName.value}`);

const subtitle = computed(
  () =>
    `Are you sure you want to disable ${fullName.value}? this will affect their access to this organization - ${props.data?.organization?.name}.`,
);
...
const submit = async () => {
  if (props.data?.organization?.id && props.data?.user_id) {
    try {
      ...
      await organizationsService.updateOrganizationUserDisable(
  ...
  >
    ...
    <template #footer>
      <div class="mm-flex mm-justify-flex-end mm-flex-gap-6">
        ...
        <m-m-button variant="danger" :loading="isLoading" @click="submit"
          >Disable user
        </m-m-button>
      </div>
    </template>
